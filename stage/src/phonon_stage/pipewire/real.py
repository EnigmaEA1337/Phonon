...
class RealPipeWireBackend:
    ...
    async def load_filter_chain(self, args: list[str]) -> int | None:
        """Load one filter-chain as a stand-alone pactl module. Args
        is the list of `key=value` strings produced by
        render_filter_chain_args(). Returns the module id on success
        — same shape as load_loopback, so the mixer can track it in
        _owned_chains and unload via unload_module() when the chain
        goes away.

        Avoids the filter-chain.service restart cascade that wipes
        every other pactl module on this stage."""
        try:
            out = await cli.run_command(
                "pactl", "load-module", "module-filter-chain", *args
            )
        except Exception:
            logger.warning(
                "pipewire.filter_chain_load_failed",
                args_preview=args[:3],
                exc_info=True,
            )
            return None
        ...
        if not out.isdigit():
            logger.warning(
                "pipewire.filter_chain_load_unexpected_output",
                output=out,
                args_preview=args[:3],
            )
            return None
        mid = int(out)
        logger.info("pipewire.filter_chain_loaded", module_id=mid, args_preview=args[:3])
        cli.invalidate_pw_dump_cache()
        return mid

    async def list_filter_chain_modules(self) -> dict[int, str]:
        """Map {module_id: chain_name} for every currently-loaded
        module-filter-chain. We tag every chain we own with
        media.name=<chain_name> so a string-match on the args column
        finds them — same pattern as list_loopback_modules.

        Returns an empty dict on parse failure — orphan cleanup is
        ...
        for line in out.splitlines():
            parts = line.split("\t")
            if len(parts) < 3 or parts[1] != "module-filter-chain":
                continue
            try:
                mid = int(parts[0])
            except ValueError:
                continue
            # Extract media.name=<value> from the args column. SPA-JSON
            # quotes the value so strip them.
            args_col = parts[2]
            for tok in args_col.split():
                if tok.startswith("media.name="):
                    val = tok.split("=", 1)[1].strip('"')
                    result[mid] = val
                    break
            else:
                # No media.name → not one of ours (some operator-loaded
                # chain). Leave it alone; we only track our own.
                continue
        return result
