...
class AirplayV1Settings(PluginSettings):
    ...
    nqptp_interface: str = Field(
        default="",
        max_length=15,
        description=(
            "Interface nqptp binds its UDP 319/320 listener to. Empty = "
            "listen on every iface (nqptp default). Set this on multi-NIC "
            "stages to keep AirPlay 2 PTP traffic on the audio iface and "
            "off the mgmt one. Validated against the Linux iface name "
            "ruleset (1-15 chars, alnum + ._-). Only honoured in v2 mode."
        ),
    )
...
class AirplayV1Plugin:
    ...
    async def _apply_nqptp_iface(self, iface: str) -> None:
        """Install or clear the nqptp systemd drop-in that pins it to
        a specific interface. Called from any path that updates
        airplay_version=2 settings. The helper is sudo-granted; the
        plugin user can call it without prompting."""
        import asyncio
        import structlog
    ...
    def _render_conf(s: AirplayV1Settings) -> str:
        ...
        # Persist nqptp_interface as a sentinel comment — the field
        # belongs to phonon-side settings, not to shairport-sync's
        # conf grammar. We need it round-trippable through this file
        # because the file is our source of truth on get_settings().
        nqptp_iface_comment = (
            f'// phonon-nqptp-interface = "{s.nqptp_interface}"\n' if s.nqptp_interface else ""
        )
        return (
    ...
    def _parse_conf(raw: str) -> AirplayV1Settings:
        ...
        # Extract phonon sentinel comments BEFORE stripping. These
        # carry phonon-side state (e.g. nqptp_interface) that has no
        # representation in shairport-sync's own grammar.
        nqptp_m = re.search(r'//\s*phonon-nqptp-interface\s*=\s*"([^"]*)"', raw)
        nqptp_interface = nqptp_m.group(1) if nqptp_m else ""
        ...
        return AirplayV1Settings(
            ...
            nqptp_interface=nqptp_interface,
