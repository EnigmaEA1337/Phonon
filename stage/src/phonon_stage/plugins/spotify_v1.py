...
class SpotifyV1Settings(PluginSettings):
    ...
    volume_normalisation: bool = Field(
        default=False,
        description=(
            "Apply Spotify's per-track loudness normalisation. Useful in "
            "party / mixed-genre playlists where one track is mastered "
            "much louder than the next."
        ),
    )
    normalisation_pregain: float = Field(
        default=0.0,
        ge=-10.0,
        le=10.0,
        description=(
            "Level (dB) added after normalisation. Negative trims hot "
            "masters down further; positive recovers headroom on quiet "
            "albums. Only meaningful when volume_normalisation = true."
        ),
    )
    ...
    discovery: bool = Field(
        default=True,
        description=(
            "Advertise this receiver on the LAN via mDNS so Spotify apps "
            "can pick it from their Connect picker. Disable for a headless "
            "username/password-only stream."
        ),
    )
    ...
    proxy: str = Field(
        ...
        max_length=512,
        description=(
            "HTTP proxy URL for Spotify traffic (e.g. http://proxy:8080). "
            "Empty = no proxy."
        ),
    )
    username: str = Field(
        ...
        max_length=128,
        description=(
            "Spotify account username for direct login (alternative to "
            "Connect-discovery). Empty = zeroconf only."
        ),
    )
    password: str = Field(
        default="",
        max_length=128,
        description=(
            "Spotify password — only used if username is set. Stored in "
            "the spotifyd.conf file at mode 0644; not encrypted."
        ),
    )

    # ── Hooks ───────────────────────────────────────────────────
    on_song_change_hook: str = Field(
        default="",
        max_length=512,
        description=(
            "Shell command run on every track change. Receives the new "
            "track metadata via env vars (PLAYER_EVENT, TRACK_ID, etc.). "
            "Useful for Slack notifications, logging, scene triggers."
        ),
    )
...
class SpotifyV1Plugin:
    ...
    def _render_conf(s: SpotifyV1Settings) -> str:
        ...
        Optional string fields (proxy, username, password,
        on_song_change_hook) are emitted only when non-empty so an
        empty value falls back to spotifyd's own default rather than
        being pinned to "".
        """
        body = (
            ...
            f"volume_normalisation = {_toml_bool(s.volume_normalisation)}\n"
            f"normalisation_pregain = {s.normalisation_pregain}\n"
            f"discovery = {_toml_bool(s.discovery)}\n"
            f"zeroconf_port = {s.zeroconf_port}\n"
        )
        if s.proxy:
            body += f'proxy = "{_toml_escape(s.proxy)}"\n'
        if s.username:
            body += f'username = "{_toml_escape(s.username)}"\n'
        if s.password:
            body += f'password = "{_toml_escape(s.password)}"\n'
        if s.on_song_change_hook:
            body += f'on_song_change_hook = "{_toml_escape(s.on_song_change_hook)}"\n'
        return body
    ...
    def _parse_conf(raw: str) -> SpotifyV1Settings:
        ...
        vn = _extract_bool(raw, "volume_normalisation")
        if vn is not None:
            out["volume_normalisation"] = vn
        npg = _extract_float(raw, "normalisation_pregain")
        if npg is not None:
            out["normalisation_pregain"] = npg
        disc = _extract_bool(raw, "discovery")
        if disc is not None:
            out["discovery"] = disc
        prx = _extract_string(raw, "proxy")
        if prx is not None:
            out["proxy"] = prx
        usr = _extract_string(raw, "username")
        if usr is not None:
            out["username"] = usr
        pwd = _extract_string(raw, "password")
        if pwd is not None:
            out["password"] = pwd
        hook = _extract_string(raw, "on_song_change_hook")
        if hook is not None:
            out["on_song_change_hook"] = hook
        try:
...
_FLOAT_RE_TMPL = r"^\s*{key}\s*=\s*(-?\d+(?:\.\d+)?)\s*$"
...
def _extract_float(raw: str, key: str) -> float | None:
    m = re.search(_FLOAT_RE_TMPL.format(key=re.escape(key)), raw, re.MULTILINE)
    if not m:
        return None
    try:
